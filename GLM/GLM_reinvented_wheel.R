# > GLM_reinvented_wheel.R <
# 
# Original script was written by Sylvain Foret (RIP), used in his honeybee
# projects.
#
# I modified the script to take in replicate data (Sylvain's original code
# pooled replicates together, thus "reinventing the wheel", in my boss' words).
# 
# Other modifications include having a different set of sanity checks--his
# original checks are commented out under load_meth_table(). Mine resides in
# do_GLM_gene(), and I elaborated on my thought processes in the function.
#
# These modifications were small, to an otherwise perfectly functional
# (and incredibly aesthetically pleasing) code base, so credit should mainly
# go to Sylvain.
...
sub_meth_table <- meth_table[meth_table$treatment == 'pH_7.2' | meth_table$treatment == 'pH_8.1', ]
genes <- do_GLM_gene(sub_meth_table, min_cpg=5)
write.table(genes, 'reinv.7.2v8.1.5cpg.tsv', sep='\t', row.names=FALSE)

sub_meth_table <- meth_table[meth_table$treatment == 'pH_7.4' | meth_table$treatment == 'pH_8.1', ]
genes <- do_GLM_gene(sub_meth_table, min_cpg=5)
write.table(genes, 'reinv.7.4v8.1.5cpg.tsv', sep='\t', row.names=FALSE)

sub_meth_table <- meth_table[meth_table$treatment == 'pH_7.8' | meth_table$treatment == 'pH_8.1', ]
genes <- do_GLM_gene(sub_meth_table, min_cpg=5)
write.table(genes, 'reinv.7.8v8.1.5cpg.tsv', sep='\t', row.names=FALSE)